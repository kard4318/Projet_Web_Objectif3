...
- `app.py` – Flask API for chat + sources (Groq backend).
- `connect_memory_with_llm.py` – CLI prototype using a Groq model + FAISS retrieval.
...
| `app.py` | Flask API exposing the RAG chatbot |
...
python app.py
```
The API will be available at http://localhost:5000 (endpoint: `/chat`).

## 🔄 Switching Embedding Modes
Both scripts can be adapted to call a HuggingFace Inference endpoint for embeddings instead of downloading the model locally. Swap `get_vectorstore()` for a variant using `HuggingFaceEmbeddings` with `api_token`.
...
python app.py                              # Flask API (needs GROQ_API_KEY)
